...
Микросервис для работы с событиями.

Данный микросервис позволяет создавать события, получать события по id, обновлять события по id, получать список всех событий с пагинацией и необязательными фильтрами по владельцу и статусу регистрации, удалять событие по id, а также добавлять пользователей в команду события и удалять их из команды, обновлять роль пользователя в команде события и получать список всей команды указанного события.

Модель Event включает следующие поля: name, description, createdDateTime, startDateTime, endDateTime, location, ownerId, registrationStatus (OPEN, CLOSED, SUSPENDED), participantLimit. Помимо этого, модель Event включает команду организаторов. У организатора может быть одна из двух ролей: исполнитель/менеджер.

В микросервисе реализован feign client для обращения к user-service с целью проверки на существование пользователя при создании мероприятия (проверка ownerId) и при добавлении участника в команду (проверка всех переданных участников).
...
## Эндпоинты:
• ***POST*** _/events_ - создание события;

• ***PATCH*** _/events/{id}_ - обновление события;

• ***GET*** _/events/{id}_ - получение события по id;

• ***GET*** _/events?page={page}&size={size}&ownerId={ownerId}&registrationStatus={registrationStatus}_ - получение списка всех событий с пагинацией и необязательными фильтрами по владельцу и статусу регистрации на событие;

• ***DELETE*** _/events/{id}_ - удаление события по id;

• ***POST*** _/events/teams_ - добавление пользователя в команду;

• ***GET*** _/events/teams/{eventId}_ - получение всей команды указанного события;

• ***PATCH*** _/events/teams/{eventId}/{memberId}_ - обновление роли пользователя в команде события;

• ***DELETE*** _/events/teams/{eventId}/{memberId}_ - удаление пользователя из команды события;

## Запустить микросервис можно двумя способами.
### Первый способ:
•	Предварительно создать БД POSTGRES не ниже версии 15 с именем "_events-db_".
...
•	Запустить микросервис командой java `java -jar event-service-1.0.0-SNAPSHOT.jar`.

•	Микросервис будет доступен по адресу: _http://localhost:8070_.
